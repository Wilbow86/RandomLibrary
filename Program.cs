...
class Program()
{
    public static void Main()
    {
        ...
        int[] w = [1, 10000, 1, 1, 10, 1, 2, 1, 1, 1, 5, 1, 1, 1, 1, 1];
        DistTable tab = new DistTable(w);
        int[] ou;
        ou = BinomialDist.rollWithoutReplacement(tab, w.Length);
        for (int i = 0; i < ou.Length; i++)
        {
            Console.WriteLine( ou[i] );
        }
